...
import com.sirisdevelopment.telegram.tgnet.AbstractSerializedData;
...
import com.sirisdevelopment.telegram.tgnet.TLClassStore;
...
public class SuperChatHelper {
    ...
    private static volatile SuperChatHelper Instance = null;

    public static SuperChatHelper getInstance() {
        SuperChatHelper localInstance = Instance;
        if (localInstance == null) {
            synchronized (SuperChatHelper.class) {
                localInstance = Instance;
                if (localInstance == null) {
                    Instance = localInstance = new SuperChatHelper();
                }
            }
        }
    ...
    protected void processUpdateEncryption(TLRPC.TL_updateEncryption update, ConcurrentHashMap<Integer, TLRPC.User> usersDict) {
        ...
        if (newChat instanceof TLRPC.TL_encryptedChatRequested && existingChat == null) {
            ...
            SuperChatHelper.getInstance().acceptSecretChat(newChat);
            ...
            if (existingChat != null && existingChat instanceof TLRPC.TL_encryptedChatWaiting && (existingChat.auth_key == null || existingChat.auth_key.length == 1)) {
                ...
                SuperChatHelper.getInstance().processAcceptedSecretChat(newChat);
